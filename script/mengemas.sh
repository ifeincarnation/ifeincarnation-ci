...
aosp=$(ls out/target/product/$device/aosp_*-ota-*.zip || true)
...
file_name=$(basename out/target/product/$device/AospExtended-v7.3-*.zip)
DL_LINK=$GDRIVE_INDEX/$name_rom/$device/$file_name
rclone copy out/target/product/$(grep unch $CIRRUS_WORKING_DIR/build.sh -m 1 | cut -d ' ' -f 2 | cut -d _ -f 2 | cut -d - -f 1)/AospExtended-v7.3-*.zip zack:$(grep init $CIRRUS_WORKING_DIR/build.sh -m 1 | cut -d / -f 4)/$(grep unch $CIRRUS_WORKING_DIR/build.sh -m 1 | cut -d ' ' -f 2 | cut -d _ -f 2 | cut -d - -f 1) -P
...
time com ccache 1
rclone copy --drive-chunk-size 256M --stats 1s ccache.tar.gz zack:ccache/$name_rom/$device -P
rm -rf ccache.tar.gz
echo ━━━━━━━━━ஜ۩۞۩ஜ━━━━━━━━
